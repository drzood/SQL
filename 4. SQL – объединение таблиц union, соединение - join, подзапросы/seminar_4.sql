...
указав их id, имя и фамилию, город и аватарку 
...
(используя вложенные запросы) */
SELECT filename FROM media WHERE
(SELECT id FROM users WHERE email='arlo50@example.org')=user_id
AND media_type_id IN (SELECT id FROM media_types WHERE name_type='Photo');

/* Задача 3: выбрать id друзей 
пользователя с id = 1 (используя UNION) */
SELECT target_user_id FROM friend_requests WHERE 
initiator_user_id = 1 AND status = 'approved'
UNION
SELECT initiator_user_id FROM friend_requests WHERE 
target_user_id = 1 AND status = 'approved';

/* Задача 4: выбрать всех пользователей, 
указав их id, имя и фамилию, город и 
аватарку (используя JOIN) */
SELECT u.id,
CONCAT(u.firstname, ' ', u.lastname) AS 'username',
p.hometown AS 'town',
m.filename AS 'avatarka'
FROM users AS u JOIN profiles p ON u.id=p.user_id
LEFT JOIN media m ON p.photo_id = m.id;

/* Задача 5: Список медиафайлов пользователей 
с количеством лайков (используя JOIN) */
SELECT m.id, m.filename AS 'File',
CONCAT(u.firstname, " ", u.lastname) AS "Player",
COUNT(l.id) AS 'like' FROM media m
LEFT JOIN likes l ON l.media_id = m.id
JOIN users u ON u.id = m.user_id
GROUP BY m.id ORDER BY m.id;

/* Задача 6: Список медиафайлов пользователей, 
указав название типа медиа (id, filename, 
name_type) (используя JOIN) */
SELECT m.id, m.filename AS 'media', mt.name_type AS 'Tip'
FROM media m LEFT JOIN media_types mt ON mt.id = m.media_type_id
ORDER BY m.id;